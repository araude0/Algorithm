...
public class test {
    ...
    public static void process(int x, int y, int index) {
        ...
        ArrayList<PositionT> united = new ArrayList<>();
        united.add(new PositionT(x, y));
        // 너비 우선 탐색 (BFS)을 위한 큐 라이브러리 사용
        Queue<PositionT> q = new LinkedList<>();
        q.offer(new PositionT(x, y));
        unions[x][y] = index; // 현재 연합의 번호 할당
        int summary = graph[x][y]; // 현재 연합의 전체 인구 수
        int count = 1; // 현재 연합의 국가 수
        // 큐가 빌 때까지 반복(BFS)
        while (!q.isEmpty()) {
            PositionT pos = q.poll();
            x = pos.getX();
            y = pos.getY();
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (0 <= nx && nx < n && 0 <= ny && ny < n && unions[nx][ny] == -1) {
                    ...
                    if (l <= gap && gap <= r) {
                        q.offer(new PositionT(nx, ny));
                        // 연합에 추가하기
                        unions[nx][ny] = index;
                        summary += graph[nx][ny];
                        count += 1;
                        united.add(new PositionT(nx, ny));
                    }
                }
            }
        }
